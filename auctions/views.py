...
def create_listing(request):
    if request.method == "POST":
        ...
        bid.save()
        listing = Listing(title=title, owner=user, bid=bid, description=description, image_url=image_url, category=category, is_closed=False)
        listing.save()
...
def display_watchlist(request):
    user = request.user
    lsitings = user.watch_listings.all()
    context = {
        "listings": lsitings
    }
